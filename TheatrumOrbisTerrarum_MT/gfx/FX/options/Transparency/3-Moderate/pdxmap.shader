...
float3 calculate_secondary( float2 uv, float3 vColor, float2 vPos )
{
	...
	return lerp( vColor, vSample.rgb, saturate( vSample.a * vMask.a ) )*1.25f; //regulates brightness of colored map modes (<1.0 = darker, >1.0 = brighter)
}
	...
	[[
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
		...
		#endif
			{
                ...
                vFoWColor = vFoWColor * .57f; // Kryo's edit for terrain fog of war
		...
		#endif
			{
				...
				//colorRange = .15f * colorRange + .4f;
				//colorRange = .2f * colorRange + .375f; //Medium Opacity
				//colorRange = .35f * colorRange + .2f; //Medium Opacity
				colorRange = .1f * colorRange + .24f; //Medium Opacity
				//float2 vBlend = float2( .70f, .3f ); // Transparent version
				float2 vBlend = float2( 1.02f - colorRange ,  colorRange); // Dynamic version (.95 in negi version)
				vOut = (vTerrainDiffuseSample.rgb * vBlend.x + vColorMapSample.rgb * vBlend.y);
				vOut = CalculateMapLighting( vOut, vHeightNormalSample )*1.15f; //regulates brightness of colored mapmodes on all zoom stages
