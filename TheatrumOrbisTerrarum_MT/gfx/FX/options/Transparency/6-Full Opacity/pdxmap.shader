...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			float3 vHeightNormalSample = normalize( tex2D( HeightNormal, Input.uv2 ).rbg - 0.5f )*0.77; //added *1.2 to make the snow transition smoother in colored map modes
		...
		#endif
			{
				...
				colorRange = .2f * colorRange + .55f; //Even More Opacity
				float2 vBlend = float2( .01f, .85f ); // Transparent version
				...
				vOut = CalculateMapLighting( vOut, vHeightNormalSample )*1.1f; //regulates brightness of colored mapmodes on all zoom stages
