...
float mipmapLevel( float2 uv )
{
    ...
    float d = max( dot(dx, dx), dot(dy, dy) );
    //return 0.5f * log2( d );
    return 0.5f * log2( d ); // reducing 0.5 increases flickering, increased to 0.6 to reduce flickering
#endif //PDX_OPENGL
}
...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			float3 vHeightNormalSample = normalize( tex2D( HeightNormal, Input.uv2 ).rbg - 0.5f )*0.75f; //added *1.2 to make the snow transition smoother in colored map modes
		...
		#endif
			{
				...
				colorRange = .15f * colorRange + .375f;
				//float2 vBlend = float2( .70f, .3f ); // Transparent version
				float2 vBlend = float2( 0.95f - colorRange ,  colorRange); // Dynamic version (.95 in negi version)
