...
float3 ApplyDistanceFog( float3 vColor, float3 vPos )
{
	...
	float vFogFactor = 0.9f - abs( normalize( vDiff ).y ); // abs b/c of reflections // 1.0f
...
float GetFoW( float3 vPos, float4 vFoWColor, in sampler2D FoWDiffuse )
{
	...
	float vShade = vFoWDiffuse + 0.7f; // .65 to 0.7 makes fow brighter
	float vIsFow = vFoWColor.a;
	return lerp( 1.075f, saturate( vIsFow + vShade ), vFoWOpacity_Time.x );
}
...
const static float3 SNOW_COLOR = float3( 0.8f, 0.7f, 0.55f ); // 0.7f, 0.7f, 0.7f
