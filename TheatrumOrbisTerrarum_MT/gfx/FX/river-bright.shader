...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT In ) : PDX_COLOR
		{
			...
			vColor = ApplyWaterSnow( vColor, In.vPrePos_Fade.xyz, vSurfaceNormal, vFoWColor, FoWDiffuse )*2.5f; // added *2.75f to make rivers brighter in winter
			vColor = CalculateLighting( vColor, vBottomNormal )*.8f;
