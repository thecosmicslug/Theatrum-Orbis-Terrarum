...
//  
	...
	float3 terrain_normal = tex2Dlod( TerrainNormal, sample_terrain( IndexU.w, IndexV.w, vTileRepeat, vMipTexels, lod ) ).rbg - 0.5f;
			...
			colorRange = .15f * colorRange * colorRange + .325f; // originally 0.4 & 0.05
			...
			float2 vBlend = float2( 1.1f - colorRange ,  colorRange); // 0.95f
			...
			vOut = CalculateMapLighting( vOut, normal ); // added *.75 to make colored map modes darker
